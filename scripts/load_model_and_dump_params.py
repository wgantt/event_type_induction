...
def main(args):
    if args.all:
        ...
        outfiles = [os.path.join(args.out_path, name.split(".")[0] + ".csv") for name in model_names]
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("model_path", type=str, help="Path to the model whose parameters are to be dumped")
    parser.add_argument("out_path", type=str, help="CSV file to which model parameters are to be dumped")
    parser.add_argument("--all", action="store_true", help="If specified, process an entire directory of model checkpoints")
